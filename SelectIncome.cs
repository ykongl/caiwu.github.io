...
namespace 财务管理
{
    public partial class SelectIncome : Form
    {
        public SelectIncome()
        {
            InitializeComponent();
        ...
        {
            string sql = "select Id,Name,IncomeNum,IncomeSource,IncomeDate from Income where InDelete=0";
            ...
            if (txtName.Text == "" && txtDate.Text!= "")
            {
                sql += "and IncomeDate like '%' + @date + '%'";
                list.Clear();
                list.Add(p2);
            }
            if(txtName.Text !="" && txtDate.Text!= "")
            {
                sql += "and Name like '%' + @name + '%'and IncomeDate like '%' + @date + '%'";
                list.Clear();
                list.Add(p);
                list.Add(p2);
            }
        ...
        private void SelectIncome_Load(object sender, EventArgs e)
        {
            LoadList();
        }
